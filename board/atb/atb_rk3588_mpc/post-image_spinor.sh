...
GENIMAGE_CFG="${BOARD_DIR}/genimage-${BOARD_NAME}_spinor.cfg"
...
${BUILD}/uboot-${UBOOT_REPO}/tools/mkenvimage -s ${UBOOT_ENV_SIZE} -o ${OUTPUT}/images/uboot_env_for_spinor.bin ${BOARD_DIR}/uboot/uboot_env_spinor.txt
...
mv ${IMAGES}/sdcard.img ${IMAGES}/${IMAGE_NAME}_spinor.img
...
if [ $? -eq 0 ]; then
	...
	echo
	echo "		sudo dd if=${IMAGES}/${IMAGE_NAME}_spinor.img of=/dev/sdX status=progress bs=1M && sync"
	echo
