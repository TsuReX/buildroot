...
GENIMAGE_DEBIAN_CFG="${BOARD_DIR}/genimage-${BOARD_NAME}_sdcard_debian.cfg"
GENIMAGE_INITRD_CFG="${BOARD_DIR}/genimage-${BOARD_NAME}_sdcard_initrd.cfg"
...
${BUILD}/uboot-${UBOOT_REPO}/tools/mkenvimage -s ${UBOOT_ENV_SIZE} -o ${OUTPUT}/images/uboot_env_for_sdcard_debian.bin ${BOARD_DIR}/uboot/uboot_env_sdcard_debian.txt
${BUILD}/uboot-${UBOOT_REPO}/tools/mkenvimage -s ${UBOOT_ENV_SIZE} -o ${OUTPUT}/images/uboot_env_for_sdcard_initrd.bin ${BOARD_DIR}/uboot/uboot_env_sdcard_initrd.txt
#echo "finish to generate uboot environments"

export PATH=$PATH:/sbin

cp ${IMAGES}/uboot_env_for_sdcard_debian.bin ${IMAGES}/uboot.env
${OUTPUT}/host/bin/genimage \
	--rootpath "${ROOTPATH_TMP}"   \
	--tmppath "${GENIMAGE_TMP}"    \
	--inputpath "${BINARIES_DIR}"  \
	--outputpath "${BINARIES_DIR}" \
	--config "${GENIMAGE_DEBIAN_CFG}"
mv ${IMAGES}/sdcard.img ${IMAGES}/${IMAGE_NAME}_debian.img


rm -rf "${GENIMAGE_TMP}"
cp ${IMAGES}/uboot_env_for_sdcard_initrd.bin ${IMAGES}/uboot.env
${OUTPUT}/host/bin/genimage \
	--rootpath "${ROOTPATH_TMP}"   \
	--tmppath "${GENIMAGE_TMP}"    \
	--inputpath "${BINARIES_DIR}"  \
	--outputpath "${BINARIES_DIR}" \
	--config "${GENIMAGE_INITRD_CFG}"

mv ${IMAGES}/sdcard.img ${IMAGES}/${IMAGE_NAME}_buildroot.img
...
if [ $? -eq 0 ]; then
	...
	echo
	echo "		sudo dd if=${IMAGES}/${IMAGE_NAME}_debian.img of=/dev/sdX status=progress bs=1M && sync"
    echo "OR"
 	echo "		sudo dd if=${IMAGES}/${IMAGE_NAME}_buildroot.img of=/dev/sdX status=progress bs=1M && sync"   
	echo
