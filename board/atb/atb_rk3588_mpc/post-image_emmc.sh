...
GENIMAGE_CFG="${BOARD_DIR}/genimage-${BOARD_NAME}_emmc.cfg"
...
#UART_TPL_BIN=uart_1500000_${TPL_BIN}
#sed 's/uart baudrate=/uart baudrate=1500000/g' ${BUILD}/rkbin/tools/ddrbin_param.txt > ${BUILD}/rkbin/tools/ddrbin_param_1500000.txt
...
cp ${BUILD}/rkbin/bin/rk35/rk3588_bl31_v1.25.elf ${OUTPUT}/build/uboot-${UBOOT_REPO}/bl31.elf
cp ${BUILD}/rkbin/bin/rk35/rk3588_bl32_v1.10.bin ${OUTPUT}/build/uboot-${UBOOT_REPO}/tee.bin
...
echo "start to generate uboot environments"
#uboot environment offset. Must be equal with CONFIG_ENV_SIZE paramter in uboot .config
UBOOT_ENV_SIZE=0x8000
${BUILD}/uboot-${UBOOT_REPO}/tools/mkenvimage -s ${UBOOT_ENV_SIZE} -o ${OUTPUT}/images/uboot_env_for_emmc.bin ${BOARD_DIR}/uboot/uboot_env_emmc.txt
echo "finish to generate uboot environments"
export PATH=$PATH:/sbin

cp ${IMAGES}/uboot_env_for_emmc.bin ${IMAGES}/uboot.env
...
mv ${IMAGES}/sdcard.img ${IMAGES}/${IMAGE_NAME}_emmc.img

cp ${BOARD_DIR}/rkdeveloptool ${IMAGES}/rkdeveloptool
cp ${BOARD_DIR}/rk3588_spl_loader_v1.09.111.bin ${IMAGES}/rk3588_spl_loader_v1.09.111.bin
if [ $? -eq 0 ]; then
	echo
	echo "Now file image was created successfully."
	echo "if you want to flash emmc you can follow the next steps:"
	echo "1) power down the target board"
	echo "2) disable boot from emmc, sdcard, spinor by jumpers"
	echo "3) power on the target board"
	echo "4) put jumper to enable boot from emmc"
	echo "5) put the next commands in desktop terminal (not target board!):"
	echo "5.1) lsusb"
	echo "make sure that rockchip entered the MaskRom state:"
	echo "Bus 001 Device 038: ID 2207:350b Fuzhou Rockchip Electronics Company"
	echo "5.2) sudo ./rkdeveloptool db rk3588_spl_loader_v1.09.111.bin"
	echo "make sure that miniloader successfully download on DDR:"
	echo  "Downloading bootloader succeeded"
	echo "5.3) sudo ./rkdeveloptool ef"
	echo "make sure that emmc completely erased:"
	echo "Erasing flash complete."
	echo "5.4) sudo ./rkdeveloptool wl 0x0 ${IMAGE_NAME}_emmc.img"
	echo "make sure that emmc completely updated:"
	echo "Write LBA from file (100%)"
	echo "This bootable image will contain MBR with U-Boot, boot fat32 partition with Linux kernel,"
